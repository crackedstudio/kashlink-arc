...
import { EXPIRY_OPTIONS, type FeeParams, MAX_SLOTS, maxAmountEach, quoteWith, shortfall } from '../lib/links'
...
const short = computed(() => {
  ...
  return shortfall(quote.value, props.balance, props.usdcBalance ?? props.balance, props.gasReserve)
})
...
const max = computed(() => {
  ...
  return maxAmountEach(props.fees, props.token, slots.value, props.balance, props.usdcBalance ?? props.balance, props.gasReserve)
})
...
function submit() {
  if (canContinue.value) emit('continue', amountEach.value, slots.value, expiry.value)
}
...
<template>
  <main class="screen">
    ...
    <p v-if="balance !== null && !max && fees" class="hint error">
      This wallet can't fund a link yet: it needs {{ token.symbol }} for the amount<template v-if="!isNative(token)"> and</template><template v-else>,</template> USDC for {{ formatUsdc(fees.stipend * BigInt(slots)) }} of claim gas and about {{ formatUsdc(gasReserve) }} of network fee.
    </p>
    <div class="display">
      <label class="amount">
