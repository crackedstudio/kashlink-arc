 ...
 */
export function maxAmountEach(params: FeeParams, token: Token, slots: number, balance: bigint, usdcBalance: bigint, gasReserve: bigint): bigint {
  ...
  while (each > 0n) {
    const q = quoteWith(params, token, each, slots)
